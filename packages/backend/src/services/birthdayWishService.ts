...
export const getBirthdayWishById = async (
	id: number
): Promise<BirthdayWish | null> => {
	return prisma.birthdayWish.findUnique({
		where: { id },
	});
};

export const createBirthdayWish = async (birthdayWishData: {
	userId: number;
	scheduledDate: Date;
	status?: boolean;
...
export const updateBirthdayWish = async (
	id: number,
	birthdayWishData: { scheduledDate?: Date; status?: boolean }
): Promise<BirthdayWish> => {
